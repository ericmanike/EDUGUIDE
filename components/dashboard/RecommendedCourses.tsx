...
export const RecommendedCourses: React.FC<RecommendedCoursesProps> = ({
  ...
  const [activePathTitle, setActivePathTitle] = useState<string>("");
  ...
  useEffect(() => {
    const loadModules = async () => {
      ...
      try {
        ...
        let moduleProgressMap: Record<string, number> = {};
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        <div className="py-12 bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center justify-center">
          <LogoSpinner size="sm" text="Loading course modules..." />
        </div>
      ) : (
